...
class RectangularRoom(object):
    ...
    def __init__(self, width, height):
        ...
        for x in range(width):
            for y in range(height):
                temp_pos = Position(x, y)
                self.tiles[temp_pos.__str__()] = False
        #print(self.tiles)
    ...
    def cleanTileAtPosition(self, pos):
        ...
        y = int(pos.getY())
        temp_pos = Position(x, y)
        self.tiles[temp_pos.__str__()] = True
        #raise NotImplementedError
    ...
    def isTileCleaned(self, m, n):
        ...
        """
        temp_pos = Position(m, n)
        if self.isPositionInRoom(temp_pos) and self.tiles[temp_pos.__str__()]:
            return True
        else:
            return False
        #raise NotImplementedError
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        try_new_position = self.position.getNewPosition(self.getRobotDirection(), self.speed)
        if not self.room.isPositionInRoom(try_new_position):
            new_direction = random.randrange(360)
            self.setRobotDirection(new_direction)
        else:
            new_position = try_new_position
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for j in range(num_trials):
        ...
        for n in range(num_robots):
            robots.append(robot_type(sim_room, speed))
    
        steps = 0
        while sim_room.getNumCleanedTiles() < sim_room.getNumTiles() * min_coverage:
            steps += 1
            #anim.update(sim_room, robots)
            for x in robots:
                x.updatePositionAndClean()
        time_steps.append(steps)
    #anim.done()
    #print(time_steps)
    mean_time_step = sum(time_steps)/len(time_steps)
    return mean_time_step
    #raise NotImplementedError
