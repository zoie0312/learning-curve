...
class NotTrigger(Trigger):
    def __init__(self, trig):
        self.target_trigger = trig
        

    def evaluate(self, story):
        if not self.target_trigger.evaluate(story):
            return True
        else:
            return False
...
class AndTrigger(Trigger):
    def __init__(self, trig1, trig2):
        self.compare_trigger1 = trig1
        self.compare_trigger2 = trig2
        

    def evaluate(self, story):
        if self.compare_trigger1.evaluate(story) and self.compare_trigger2.evaluate(story):
            return True
        else:
            return False
...
class OrTrigger(Trigger):
    ...
    def evaluate(self, story):
        if self.compare_trigger1.evaluate(story) or self.compare_trigger2.evaluate(story):
            return True
        else:
            return False
...
class PhraseTrigger(Trigger):
    def __init__(self, phrase):
        self.phrasetomatch = phrase

    def evaluate(self, story):
        if self.phrasetomatch in story.getTitle():
            return True
        if self.phrasetomatch in story.getSubject():
            return True
        if self.phrasetomatch in story.getSummary():
            return True
        return False
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    if triggerType == 'PHRASE':
        ...
        for x in params[:-1]:
            ...
            arg += ' '
        arg += params[-1]
        triggerMap[name] = PhraseTrigger(arg)
...
def main_thread(master):
    ...
    #        if newstory.get_guid() not in guidShown:
        ...
        while True:
            ...
            #Hacker News
            #stories.extend(process("http://news.ycombinator.com/rss"))
